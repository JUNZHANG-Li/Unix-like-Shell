...
We **extend** the original language to implement **&&** and **||** sequence commands. 

Extended Grammar of **Command Line Parsing**:

    <command> ::= <pipe> | <seq> | <call>
    <pipe> ::= <call> "|" <call> | <pipe> "|" <call>
    <seq>  ::= <command> ";" <command> | <command> "&&" <command> | <command> "||" <command>
    <call> ::= ( <non-keyword> | <quoted> ) *
    
## '&&' Sequence Command

Executes a sequence of commands separated by double ampersands. For example, 

    echo hello world && cat file.txt

prints "hello world" to stdout, then displays the content of the file `file.txt`.

The syntax of this command is the following:

    <command> "&&" <command>

It runs the first command; after the first command terminates, runs the second command if and only if the first command's stderr is 1 (success). If an exception is thrown during the execution of the first command, the first command's stderr would be 0 (failed) and the next command will not be executed, the stderr remains to be the first command's stderr since the successor command was not executed.
...
It runs the first command; after the first command terminates, runs the second command if and only if the first command's stderr is 0 (failed) that is when an exception is thrown.
