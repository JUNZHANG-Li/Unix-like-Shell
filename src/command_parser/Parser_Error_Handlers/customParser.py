...
class TraceListener(ParseTreeListener):
    ...
    def visitTerminal(self, node):
        ...
        print(
            ...
            + str(node.symbol)
            + " rule "
            + self._parser.ruleNames[
                self._parser._ctx.getRuleIndex()
            ],
            file=self._parser._output,
        )
...
class customParser(Recognizer):
    ...
    # self field maps from the serialized ATN string
    # to the deserialized {@link ATN} with
    # bypass alternatives.
    #
    ...
    def __init__(
        ...
        self._errHandler = (
            customErrorStrategy()
        )  # replace original Error Handling
        self._precedenceStack = list()
        self._precedenceStack.append(0)
        # The {@link ParserRuleContext} object
        # for the currently executing rule.
        # self is always non-null during the parsing process.
        self._ctx = None
        # Specifies whether or not the parser
        # should construct a parse tree during
        # the parsing process. The default value is {@code true}.
        self.buildParseTrees = True
        # When {@link #setTrace}{@code (true)}
        # is called, a reference to the
        # {@link TraceListener} is stored
        # here so it can be easily removed in a
        # later call to {@link #setTrace}{@code (false)}.
        # The listener itself is
        # implemented as a parser listener
        # so self field is not directly used by
    ...
    def match(self, ttype: int):
        ...
        if t.type == ttype:
            ...
            if self.buildParseTrees and t.tokenIndex == -1:
                # we must have conjured up a new token
    ...
    def matchWildcard(self):
        ...
        if t.type > 0:
            ...
            t = self._errHandler.recoverInline(self)
            if self.buildParseTrees and t.tokenIndex == -1:
                # we must have conjured up a new
                # token during single token insertion
                # if it's not the current symbol
                self._ctx.addErrorNode(t)
    ...
    def getParseListeners(self):
        return (
            ...
            else self._parseListeners
        )

    # Registers {@code listener} to receive
    # events during the parsing process.
    #
    # <p>To support output-preserving grammar
    ...
    # Tell our token source and error strategy
    # about a new way to create tokens.#
    ...
    def getATNWithBypassAlts(self):
        ...
        if serializedAtn is None:
            raise UnsupportedOperationException(
                ...
                " an ATN with bypass alternatives."
            )
        result = self.bypassAltsAtnCache.get(serializedAtn, None)
        if result is None:
            deserializationOptions = ATNDeserializationOptions()
            deserializationOptions.generateRuleBypassTransitions = (
                True
            )
            result = ATNDeserializer(
    ...
    # ParseTreePattern p = parser.
    # compileParseTreePattern("&lt;ID&gt;+0", MyParser.RULE_expr);
    ...
    def notifyErrorListeners(
        ...
        listener.syntaxError(
            self, offendingToken, line, column, msg, e
        )
    ...
    def consume(self):
        ...
        hasListener = (
            self._parseListeners is not None
            and len(self._parseListeners) > 0
        )
    ...
    def getExpectedTokens(self):
        return self._interp.atn.getExpectedTokens(
            self.state, self._ctx
        )
    ...
    # Get a rule's index (i.e., {@code RULE_ruleName} field)
    # or -1 if not found.#
    ...
    def dumpDFA(self):
        ...
        for i in range(0, len(self._interp.decisionToDFA)):
            ...
            if len(dfa.states) > 0:
                ...
                print(
                    "Decision " + str(dfa.decision) + ":",
                    file=self._output,
                )
                print(
                    dfa.toString(
                        self.literalNames, self.symbolicNames
                    ),
                    end="",
                    file=self._output,
                )
    ...
    # During a parse is sometimes useful to
    # listen in on the rule entry and exit
    # events as well as token matches.
    # self is for quick and dirty debugging.
    #
